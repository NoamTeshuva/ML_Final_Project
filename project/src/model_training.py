...
train_df["Stock_Return"] = train_df["Close"].pct_change()
cluster_performance = train_df.groupby("Cluster")["Stock_Return"].mean()
threshold = cluster_performance.median()
train_df["Cluster_Strength"] = train_df["Cluster"].map(lambda c: 1 if cluster_performance[c] > threshold else 0)

# ✅ Add Cluster Strength as a Feature
train_df.loc[:, "Cluster_Strength"] = train_df["Cluster_Strength"]
feature_cols.append("Cluster_Strength")  # Ensure it's part of feature set
...
dump(scaler, os.path.join(models_dir, "scaler_with_clusters.joblib"))
...
# ✅ Ensure we're using the dataset that K-Means was trained on
X_kmeans = X_scaled  # Full dataset before PCA
print(f"✅ K-Means Silhouette Score: {silhouette_score(X_kmeans, kmeans.labels_):.2f}")
